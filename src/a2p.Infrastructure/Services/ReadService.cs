...
namespace a2p.Infrastructure.Services
{
    public class ReadService : IReadService
    {
        ...
        public async Task<List<A2POrderDto>> ReadAsync(ProgressValue progressValue, IProgress<ProgressValue>? progress = null)
        {
            _a2pOrders = [];
                        ...
                        for (int j = 0; j < _a2pOrders[i].Files.Count; j++)
                        {
                            ...
                            for (int k = 0; k < _a2pOrders[i].Files[j].Worksheets.Count; k++)
                            {
                                ...
                                if (type == WorksheetType.Items)
                                {
                                    ...
                                    {
                                        (List<DTO.ItemDTO>, List<A2PErrorDto>) result = await _mapperSapaTechDesign.MapItemsAsync(_a2pOrders[i].Files[j].Worksheets[k], _progressValue, _progress);
                                ...
                                else
                                {
                                    ...
                                    else if (_a2pOrders[i].SourceAppType == SourceAppType.TechDesign)
                                    {
                                        ...
                                        (List<DTO.MaterialDTO>, List<A2PErrorDto>) result = await _mapperSapaTechDesign.MapMaterialsAsync(_a2pOrders[i].Files[j].Worksheets[k], _progressValue, _progress);
        ...
        private async Task<List<A2POrderDto>> GetOrders(List<string>? files)
        {
        ...
        private async Task<A2POrderDto> GetOrderFilesAsync(A2POrderDto a2pOrder)
        {
                ...
                await Task.Run((Action)(() =>
                {
                    ...
                    for (int i = 0; i < files.Count; i++)
                    {
                        ...
                        if (a2pFile.IsLocked)
                        {
                            ...
                            a2pOrder.ErrorsRead.Add(new A2PErrorDto
                            {
                                Order = a2pOrder.Order,
        ...
        private async Task<A2POrderDto> GetOrderWorksheetsAsync(A2POrderDto a2pOrder)
        ...
        private async Task<A2POrderDto> GetOrderSalesDocumentAsync(A2POrderDto a2pOrder)
        {
        ...
        private async Task<A2POrderDto> GetOrderSalesDocumentState(A2POrderDto a2pOrder)
        {
            try
            {
        ...
        private A2POrderDto SetSalesDocumentReadErrors(A2POrderDto a2pOrder)
        {
            try
            {

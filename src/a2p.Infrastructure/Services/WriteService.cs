...
namespace a2p.Infrastructure.Services
{
    public class WriteService : IWriteService
    {
        ...
        public WriteService(ILogService logService, IPrefSuiteService prefSuiteService, IFileService fileService, IExcelService excelService, IMyRepository orderRepository, IPrefSuiteDataService prefSuiteDataService)
        {
        ...
        public async Task<(OrderEntity, ProgressValue)> WriteAsync(OrderEntity order, ProgressValue progressValue, IProgress<ProgressValue>? progress = null)
        {
            ...
            try
            {
                ...
                ErrorEntity? deleteError = await _prefSuiteDataService.DeleteSalesDocumentDataAsync(order.SalesDocumentNumber, order.SalesDocumentVersion, order.DeleteExistsing);
                if (deleteError != null)
                {
                ...
                var Result = await _prefSuiteService.InsertItemsAsync(order, _progressValue, _progress);
                ...
                for (int i = 0; i < order.Items.Count; i++)
                {
                    try
                    {
                        ...
                        _progress?.Report(_progressValue);
                        ErrorEntity? error = await _orderRepository.InsertOrderItemAsync(order.Items[i], order.SalesDocumentNumber, order.SalesDocumentVersion, order.Items[i].IdPos);
                        if (error != null)
                        {
                ...
                for (int i = 0; i < order.Materials.Count; i++)
                {
                    ...
                    try
                    {
                        ...
                        // Insert MaterialDTO
                        //=================================================================
                        ErrorEntity? ErrorMaterialDTO = await _orderRepository.InsertOrderMaterialAsync(order.Materials[i], order.SalesDocumentNumber, order.SalesDocumentVersion);
                ...
                ErrorEntity? errorMaterialNeedsMaster = await _prefSuiteDataService.InsertPrefSuiteMaterialNeedsMasterAsync(order.OrderNumber, order.SalesDocumentNumber, order.SalesDocumentVersion);
                if (errorMaterialNeedsMaster != null)
                {
                    order.Errors.Add(errorMaterialNeedsMaster);
                }

                ErrorEntity? errorMaterialNeeds = await _prefSuiteDataService.InsertPrefSuiteMaterialNeedsAsync(order.OrderNumber, order.SalesDocumentNumber, order.SalesDocumentVersion);

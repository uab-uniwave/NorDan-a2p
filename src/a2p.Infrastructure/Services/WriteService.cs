...
namespace a2p.Infrastructure.Services
{
    public class WriteService : IWriteService
    {
        ...
        public WriteService(ILogService logService, IPrefSuiteService prefSuiteService, IFileService fileService, IExcelService excelService, IPrefSuiteDataService prefSuiteDataServic, IOrderRepository orderRepository, IPrefSuiteDataService prefSuiteDataService,
        ...
        public async Task WriteAsync(ExcelOrderDto orders, ProgressValue progressValue, IProgress<ProgressValue>? progress = null)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < orders.MaterialDto.Count; i++)
                {
                    _progressValue.CurrentValue = _progressValue.CurrentValue + 1;//1pts. x 1 per material
                    _progressValue.ProgressTask2 = $"Inserting materials {i + 1} of {orders.MaterialDto.Count} into PrefSuite DB...";
                    _progressValue.ProgressTask3 = $"Material # {orders.MaterialDto[i].ReferenceBase} {orders.MaterialDto[i].Color}.";
                    _progress?.Report(_progressValue);
                }

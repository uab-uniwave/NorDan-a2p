    ...
    public class A2POrderMapper : IA2POrderMapper
    {
        public async Task<OrderDTO> MapToOrderDTOAsync(A2POrder order)
        {
            ...
            return await Task.Run(() =>
            {
                ...
                {
                    Order = order.Order,
                    Currency = order.OrderFiles
                        .SelectMany(file => file.OrderFileWorksheets)
                    ...
                    WorksheetList = string.Join("\n ", order.OrderFiles.SelectMany(file => file.OrderFileWorksheets).Select(ws => ws.Worksheet)),
                    ItemCount = order.OrderFiles.Sum(file => file.OrderFileWorksheets?.Sum(ws => ws.Items) ?? 0),

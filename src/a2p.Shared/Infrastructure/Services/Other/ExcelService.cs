...
namespace a2p.Shared.Infrastructure.Services.Other
{
    public class ExcelService : IExcelService
    {
        ...
        public ExcelService(ILogService logger)
        {
            _logger=logger;
        ...
        public async Task<List<A2PWorksheet>> GetWorksheetListAsync(List<A2PFile> a2pFiles, ProgressValue progressValue, IProgress<ProgressValue>? progress)
        {
                ...
                foreach (A2PFile a2pFile in a2pFiles)
                {
                    ...
                    try
                    {
                        ...
                        foreach (IXLWorksheet worksheet in workbook.Worksheets)
                        {
                            ...
                            a2pWorksheet.Name=worksheet.Name;
                            a2pWorksheet.FileName=a2pFile.Name;
                            a2pWorksheet.Order=a2pFile.OrderNumber;
                            a2pWorksheet.RowCount=worksheet.RowsUsed().Count()-4;
                            a2pWorksheet.WorksheetType=GetWorksheetType(a2pFile.Name, worksheet.Name);
                    ...
                    {
                        _logger.Error(ex, "ES. Unhandled error: Reading worksheet list from file ${Name}", a2pFile.Name);
                        continue;
                    }
            ...
            {
                _logger.Error(ex, "ES. Unhandled error: reading worksheet list from workbook)");
                return [];
            ...
            foreach (IXLRow row in ixlWorksheet.RowsUsed()??Enumerable.Empty<IXLRow>())
            {
                ...
                for (int col = 1; col<=totalColumns; col++) // Iterate through all columns
                {
                    ...
                    if (cell!=null&&!cell.IsEmpty())
                    {
                        ...
                        if (numericValue!=0)
                        {
                            _logger.Verbose("Extracting from cell {Cell} Numeric Value: {Double}", cell.Address.ToString()??"", numericValue);
                            cell.Value=numericValue.ToString(culture);
                            rowValues.Add(cell.Value);
                        }
                        else
                        {
                            _logger.Verbose("Extracted from cell {Cell} Text: {Text}", cell.Address.ToString()??"", cell.Value.ToString());
                            rowValues.Add(cell.Value.ToString());
                        }
                    }
                    else
                    {
                        _logger.Debug("Cell at column {Col} is empty!", col);
                        rowValues.Add(string.Empty); // Add empty string for empty cells
                    }


                }
                a2pWorksheet.Data.Add(rowValues);
                rowCount++;
            }
